...
function woocommerce_uzumnasiya()
{
    ...
    class WC_UZUMNASIYA extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            $this->id = 'uzumnasiya';
            $this->title = '';
//            $this->description = __("<script>const uzumnasiya = document.querySelector('.checkout-order-review');var str = document.querySelector('.woocommerce-Price-amount.amount bdi').innerHTML,matches = (matches = str.match(/\d+/g)).join('');let jallab = Number(matches); const pricemonth = document.createElement('div'); pricemonth.innerHTML = Math.floor(((jallab / 100 * 39) + jallab) / 12).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ');pricemonth.classList.add('pricemonth');const everypricemonth = document.createElement('div');everypricemonth.innerHTML = Math.floor(((jallab / 100 * 39) + jallab)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ');; everypricemonth.classList.add('everypricemonth'),intend.appendChild(pricemonth)</script>", 'uzumnasiya');
            ...
            $text = "";
            foreach ($this->calculate($total) as $month) {
          ...
          {$text}
            ...
            ";
            $this->icon = apply_filters('woocommerce_intend_icon', '' . $plugin_dir . 'uzumnasiya.png');
        ...
        public function generate_form($order_id)
        {
            ...
            $client = Uzumnasiya::checkClient($billing_phone);
...
//                         "imei"=>1,
            ...
            if ($client->data->status == 4) {
                ...
                $userPeriods = Uzumnasiya::calculate($order->get_total(), $client->data->buyer_id);
                $i = 1;
                ...
                $form .= "<hr><input type='submit' class='button alt' id='submit_intend_form' value='$label_pay'>";
                $form .= "<a class='button cancel' href='{$order->get_cancel_order_url()}'>$label_cancel</a></form>";
                return $form;
            } else {
                echo "<a target='_self' class='button' href='{$client->data->webview}'>Uzumnasiyada ro'yhatdan o'tish</a>";
            }
        }
